...
// Problem 1 - Give every runner a nickname

const nickNames = [];
runners.forEach(function(item){
  nickNames.push(`${item.first_name[0]}${item.first_name[1]}${item.first_name[2]}${item.last_name[0]}${item.last_name[1]}${item.last_name[2]}`)
});
console.log(nickNames);

// Problem 2 - Assign a race time that is directly proportional to donation

const raceTimes = runners.map(function(currentValue){
...
// Problem 3 - Tell the tiny donars (<$10) they need to pony up

const tinyDonar = runners.filter(function(currentValue){
  return currentValue.donation < 10;
})

tinyDonar.forEach(function(item){
  return console.log(`Yo, ${item.first_name} you need to pony up! $${item.donation}... really?? $${item.donation * 100} sounds more like it! `)
});
